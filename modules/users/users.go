...
type UserRegisterReq struct {
	Email    string `db:"email" json:"email" form:"email"`          // check pattern
	Password string `db:"password" json:"password" form:"password"` // hash
...
func (obj *UserRegisterReq) BcryptHashing() error {
	...
	// Cost เยอะ secure มากขึ้นแต่ใช้ resource มากขึ้น
	if err != nil {
...
type UserClaims struct {
	// ไม่ควรมีข้อมูล credential แค่ระบุตัวตนได้ก็พอ เพราะเป็น base64 สามารถถอดได้
	Id     string `db:"id" json:"id"`
